...
import { deployTestInbox, mpcAbiReEncodeOf, inboxInitArgs } from "../scripts/deploy-test-inbox.js";
...
describe("Inbox compact message events", { concurrency: false, timeout: 600_000 }, () => {
  it("MessageSent logs compact metadata while storage keeps full methodCall", async () => {
    ...
    await inbox.write.init(inboxInitArgs(inbox, deployer, SOURCE_CHAIN_ID), { account: deployer });
  ...
  it("MessageReceived logs compact metadata while incoming storage keeps full methodCall", async () => {
    ...
    await source.write.init(inboxInitArgs(source, deployer, SOURCE_CHAIN_ID), { account: deployer });
    ...
    await target.write.init(inboxInitArgs(target, deployer, TARGET_CHAIN_ID), { account: deployer });
