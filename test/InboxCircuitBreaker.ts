...
describe("Inbox circuit breaker and oracle guards", { concurrency: 1 }, async function () {
  ...
  before(async function () {
    ...
    await inbox.write.init(inboxInitArgs(inbox, deployer, 0n), { account: deployer });
  ...
  it("sendTwoWayMessage reverts while message processing is paused", async function () {
    ...
    await pausedInbox.write.init(inboxInitArgs(pausedInbox, deployer, 0n), { account: deployer });
    const fee = {
      constantFee: 1n,
      gasPerByte: 0n,
