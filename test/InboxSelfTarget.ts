...
describe("reject Inbox as own target", { concurrency: false, timeout: 1_200_000 }, () => {
  const setup = async () => {
  ...
  it("estimateExecutionGasForMiner reverts when targetContract is the Inbox", { timeout: 600_000 }, async () => {
    const { inbox, deployer } = await setup();
    await assert.rejects(
      () =>
        inbox.write.estimateExecutionGasForMiner(
          [SOURCE_CHAIN_ID, minedSelf(inbox.address, deployer), 1_000_000n],
